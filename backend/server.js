...
app.post("/register", async (req, res) => {
  ...
  const { error } = await supabase.from("users").insert([{
    username, password_hash: password, first_name,
    middle_name: middle_name || null, last_name, role: finalRole,
  }]);
...
app.post("/login", async (req, res) => {
  ...
  if (error || !data) return res.json({ success: false, message: "Invalid login" });
...
app.get("/user/:id", async (req, res) => {
  const { data, error } = await supabase.from("users").select("*")
    .eq("user_id", req.params.id).single();
...
app.put("/update-role", async (req, res) => {
  ...
  const { error } = await supabase.from("users").update({ role }).eq("user_id", id);
...
app.put("/update-username", async (req, res) => {
  ...
  const { data: existing } = await supabase.from("users").select("user_id")
    .eq("username", newUsername).single();

  if (existing) return res.json({ success: false, message: "Username already taken" });

  const { error } = await supabase.from("users").update({ username: newUsername }).eq("user_id", id);
  if (error) return res.json({ success: false });
  res.json({ success: true });
});
...
app.put("/update-name", async (req, res) => {
  ...
  const { error } = await supabase.from("users")
    .update({ first_name, middle_name, last_name }).eq("user_id", id);
  if (error) return res.json({ success: false });
  res.json({ success: true });
});
...
app.put("/update-password", async (req, res) => {
  ...
  const { error } = await supabase.from("users")
    .update({ password_hash: newPassword }).eq("user_id", id);
  if (error) return res.json({ success: false });
  res.json({ success: true });
});
...
app.post("/upload-profile-pic", upload.single("profilePic"), async (req, res) => {
  ...
  const { error: uploadError } = await supabase.storage
    .from("pulse-tracker").upload(fileName, req.file.buffer, {
      contentType: req.file.mimetype, upsert: true
    });

  if (uploadError) return res.json({ success: false, message: "Upload failed" });

  const { data: urlData } = supabase.storage.from("pulse-tracker").getPublicUrl(fileName);
  const publicUrl = urlData.publicUrl;

  const { error: dbError } = await supabase.from("users")
    .update({ profilePic: publicUrl }).eq("user_id", id);

  if (dbError) return res.json({ success: false });
...
app.post("/add-metric", async (req, res) => {
  ...
  const { error } = await supabase.from("metrics_data")
...
app.get("/metrics/:userId", async (req, res) => {
  const { data, error } = await supabase.from("metrics_data").select("*")
...
app.post("/save-reading", async (req, res) => {
  ...
  const { error } = await supabase.from("heart_readings")
...
app.get("/readings/:userId", async (req, res) => {
  const { data, error } = await supabase.from("heart_readings").select("*")
...
app.get("/nurse/students", async (req, res) => {
  try {
    ...
    if (studentsErr) return res.json([]);

    const results = await Promise.all(students.map(async (student) => {
      const { data: readings } = await supabase.from("heart_readings")
        .select("bpm, spo2, created_at").eq("user_id", student.user_id)
        .order("created_at", { ascending: false }).limit(1);

      const latestReading = readings && readings[0] ? readings[0] : null;

      const { data: metrics } = await supabase.from("metrics_data")
        .select("metric_type, value1, value2, unit, notes, created_at")
        .eq("user_id", student.user_id).order("created_at", { ascending: false });

      const metricMap = {};
      if (metrics) metrics.forEach(m => { if (!metricMap[m.metric_type]) metricMap[m.metric_type] = m; });
...
app.get("/admin/nurses", async (req, res) => {
  const { data, error } = await supabase
    .from("users")
    .select("user_id, username, first_name, middle_name, last_name, role")
    .eq("role", "nurse")
    .order("first_name", { ascending: true });
...
app.post("/admin/create-nurse", async (req, res) => {
  ...
  const { data: admin } = await supabase.from("users").select("role")
...
app.delete("/admin/delete-nurse/:id", async (req, res) => {
  ...
  const { data: target } = await supabase.from("users").select("role")
    .eq("user_id", nurseId).single();

  if (!target || target.role !== "nurse")
    return res.json({ success: false, message: "Target is not a nurse account" });

  const { error } = await supabase.from("users").delete().eq("user_id", nurseId);

  if (error) return res.json({ success: false, message: "Delete failed" });
  res.json({ success: true });
});
